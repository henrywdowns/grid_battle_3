...
func move_char(dir):
	...
	if temp_target_panel[0] > -1 && temp_target_panel[0] <= grid_partition && temp_target_panel[1] > -1 && temp_target_panel[1] < 3:
		...
		Events.entity_moved.emit(player_char,Global.battle_points_to_panels[player_char.character_gridpoint],Global.battle_points_to_panels[player_current_panel_gridpoint])
