...
func _action_pattern(_enemy):
	...
	var target_panel = Global.translate_points_to_coords(Global.enemy_panels.pick_random())
	while !_check_if_legal_space(target_panel):
		target_panel = Global.translate_points_to_coords(Global.enemy_panels.pick_random())
